...
use crate::pyfuture::{current_thread_id, PyFuture, ThreadId, ThrowCallback};
...
impl<W: CoroutineWaker + Send + Sync> ArcWake for Waker<W> {
    fn wake_by_ref(arc_self: &Arc<Self>) {
        if current_thread_id() == arc_self.thread_id {
            Python::attach(|gil| CoroutineWaker::wake(&arc_self.inner, gil))
        } else {
            Python::attach(|gil| CoroutineWaker::wake_threadsafe(&arc_self.inner, gil))
        }
    }
}
...
impl<W: CoroutineWaker + Send + Sync + 'static> Coroutine<W> {
    pub(crate) fn poll(&mut self, py: Python, exc: Option<PyErr>) -> PyResult<Option<Py<PyAny>>> {
        ...
        Ok(match res {
            ...
            Poll::Pending => Some(self.waker.as_ref().unwrap().inner.yield_(py)?),
        })
    }
}
